...
//图片路径
#define  ImagePath  [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"Documents/HH"]
...
- (void)setup{
    ...
    _photoScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(0, 0, SCREENWIDTH-20, 80)];
...
-(void)initlizerScrollView:(NSArray *)imgList{
    ...
    if(imgList.count<MaxItemCount){
        ...
        [btnphoto setBackgroundImage:[UIImage imageNamed:@"addImage.jpg"]  forState:UIControlStateNormal];
...
-(void)openMenu{
    //在这里呼出下方菜单按钮项
    [[NSNotificationCenter defaultCenter] postNotificationName:@"pickUpKey" object:nil];
